...
def detect_target_category(query: str) -&gt; Optional[str]:
    """从用户 query 自动推断最匹配的目标类目"""
    q_low = query.lower()
    best_cat = None
    max_hits = 0
...
def filter_by_category(rets: List, target_cat: Optional[str]) -&gt; List:
    """如果推断出了明确的类目，只保留该类目下的商品，完全剔除其他类目的不相关卡片"""
    if not target_cat:
        # 没推断出类目，返回原始去重排序后的结果
        return rets
    filtered = [
    ...
    # 至少保留 3-6 张相关卡片，绝不能少于 2 张导致空空如也
    if len(filtered) &lt; 2:
        return rets[:6]
    return filtered
...
async def stream_chat_response(req: ChatRequest):
    ...
    # 动态截断 最多 6 张卡片，不要 10 张那么多混着无关的
    retrieved = retrieved[:6]

    print(f"[SSE Final] after filter, total {len(retrieved)} products to show")
